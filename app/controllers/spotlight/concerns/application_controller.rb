  ...
  module Concerns
    ...
    module ApplicationController
      ...
      def enabled_in_spotlight_view_type_configuration?(config, *args)
        case
        when config.respond_to?(:upstream_if) && !config.upstream_if.nil? && !blacklight_configuration_context.evaluate_configuration_conditional(config.upstream_if, config, *args)
          false
        when current_exhibit.nil? || self.is_a?(Spotlight::PagesController)
          true
        else
      ...
      def field_enabled?(field, *args)
        ...
        when !field.enabled
          false
        when (field.respond_to?(:upstream_if) && !field.upstream_if.nil? && !blacklight_configuration_context.evaluate_configuration_conditional(field.upstream_if, field, *args))
          false
        when field.is_a?(Blacklight::Configuration::SortField) || field.is_a?(Blacklight::Configuration::SearchField)
          field.enabled
        when field.is_a?(Blacklight::Configuration::FacetField) || (self.is_a?(Blacklight::Catalog) && %w(edit show).include?(action_name))
          field.show
        else
